...
struct SettingView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            }.navigationTitle("应用设置").navigationBarTitleDisplayMode(.inline)
        }
    }
    ...
    private var notificationToggleView: some View {
        ...
        // 使用AppStorage 之后，就会自动管理，这里代码可以去掉
        // .onChange(of: isBellSlash) { newValue in
    ...
    private var darkModeToggleView: some View {
        Toggle(isOn: $isDark) {
            HStack {
                Image(systemName: "moon.stars").font(.system(size: 20))
                Text("深色模式")
            }
    ...
    private var colorPickerView: some View {
        ColorPicker(
            ...
            label: {
                HStack {
                    Image(systemName: "paintbrush").font(.system(size: 20))
                    Text("主题颜色")
                }
            })
    }
    ...
    private var fontSizeStepperView: some View {
        Stepper(
            value: $selectedNumber, in: 1...48,
            label: {
                HStack {
                    Image(systemName: "a.circle").font(.system(size: 20))
    ...
    private var feedbackBtnView: some View {
        Button(action: {
            ...
            HStack {
                Image(systemName: "ellipsis.message").font(.system(size: 20))
                Text("意见反馈")
            }
    ...
    private var aboutAsBtnView: some View {
        Button(action: {
            ...
            HStack {
                Image(systemName: "persion").font(.system(size: 20))
                Text("关于应用")
            }
        }.buttonStyle(PlainButtonStyle())
    }
